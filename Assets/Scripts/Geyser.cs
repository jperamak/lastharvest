...
namespace Assets.Scripts
{
    public class Geyser : MonoBehaviour
    {
        ...
        public void Awake()
        {
            ...
            {
                Debug.Log("Duration has to be shorter than interval");
            }
        ...
        public void Update()
        {
            ...
            if (_isSprouting)
            {
                ...
                {
                    xSize = Mathf.Lerp(size.x, _originalScale.x, (_sproutingTime - _timeToFullSize) / _timeToFullSize);
                    ySize = Mathf.Lerp(size.y, _originalScale.y, (_sproutingTime - _timeToFullSize) / _timeToFullSize);
                    transform.position = transform.position + new Vector3((xSize - transform.localScale.x) / 2.0f, (ySize - transform.localScale.y) / 2.0f);
                }
